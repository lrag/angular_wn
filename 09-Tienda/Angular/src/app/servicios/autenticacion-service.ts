    ...
    public login(login:string, password:string):Observable<any>{
        ...
        return this.httpClient.post(ConfiguracionUtil.urlServidor+"/login", { login:login, password:password })
            .pipe(
                ...
                ,catchError( (error:any, observable:Observable<any>) => {
                    console.log("Error capturado en autenticacionService:", error)
                    return new Observable(subscribers => subscribers.error("OTRO ERROR"))
                })
            )
